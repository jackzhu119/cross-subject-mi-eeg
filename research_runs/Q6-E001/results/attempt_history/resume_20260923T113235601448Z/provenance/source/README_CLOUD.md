...
- PyTorch 2.14.0 with CUDA 13.2 wheels on the RTX 5090 host
- Braindecode 1.5.1
- Scientific Python package versions are pinned in `requirements-cloud.txt`.

The current cloud instance is an RTX 5090 (compute capability `sm_120`) with a CUDA 13.2 / PyTorch 2.13.0 / Python 3.12 base image. The base image passed a real CUDA matrix multiplication on this host. `setup_gpu.sh` installs the official PyTorch 2.14.0 CUDA 13.2 wheel in an isolated environment so the experiment retains the planned PyTorch version and supports this GPU architecture. The original uploaded ZIP used a CUDA 12.6 wheel; its untouched installer is preserved as `setup_gpu.cu126.original.sh` on the cloud host. Only the CUDA runtime wheel index changed: Q5 model, preprocessing, data split, seeds, epoch-selection rule, and metrics are unchanged. Device availability is checked again inside the isolated environment before training. No GPU test was performed on the local Windows computer.
...
- Official PyTorch CUDA 13.2 wheel index for Python 3.12 Linux x86_64: https://download.pytorch.org/whl/cu132/torch/
