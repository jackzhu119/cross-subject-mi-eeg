"""Resumable Q10-E001 source-fitted spatial projection EEGNet runner.

Run selection for a spatial condition before final inference. Every inner fit
...
36 inner fits exist. Q4--Q9 artifacts are read-only.

Examples::

    python scripts/q10_spatial_neural.py --condition MU_BETA_CSP8_EEGNET --phase selection \
        --data-dir data/raw --device cuda
    python scripts/q10_spatial_neural.py --condition MU_BETA_CSP8_EEGNET --phase final \
        --data-dir data/raw --device cuda
...
PARENT_MATRIX = ROOT / "research_runs/Q9-E001/Q9_BATCH_MATRIX.json"
...
CONDITIONS = {
    "MU_BETA_CSP8_EEGNET": ("Q10-E001", {"mu": [8, 13], "beta": [13, 30]}, False),
    "MU_BETA_PCA8_EEGNET": ("Q10-E001", {"mu": [8, 13], "beta": [13, 30]}, False),
}
...
def _locked_settings(condition: str, data_dir: Path, output: Path) -> tuple[dict, dict]:
    ...
    if row is None or row["experiment_id"] != experiment_id:
        raise AssertionError("Condition does not match the Q10-E001 matrix")
    ...
    if experiment_id != "Q10-E001" or shared:
        raise AssertionError("This runner is restricted to the Q10-E001 spatial pair")
    pre = dict(q8["preprocessing"])
    pre["bands"] = bands
    # The target population is the same include-all population as frozen Q8/Q9.
    pre["artifact_policy"] = "include_all"
    architecture = dict(q8["architecture"])
    architecture["n_chans"] = 16
    spatial_method = "csp8" if condition == "MU_BETA_CSP8_EEGNET" else "pca8"
    config = {
        ...
        "spatial_input_units": "volts",
        "model_input_units": "source_standardized_dimensionless",
        "spatial_helper_sha256": sha256_file(ROOT / "scripts/q9_spatial_features.py"),
        "source_only_selection": True,
        "fixed_q8_epochs": False,
        "source_training_artifact_exclusion": False,
    ...
    if lock_file.exists():
        ...
        if old != config:
            raise AssertionError("Existing Q10 run_config differs; refusing to mix protocols")
    else:
...
def _fit_manifest(meta: pd.DataFrame, target: int, stage: str, inner_fold: int | None,
    ...
    train_ids = {int(i) for i in train}
    val_ids = set() if validation is None else {int(i) for i in validation}
    ...
    for subject in range(1, 10):
        idx = {int(i) for i in np.flatnonzero((meta.subject == subject).to_numpy())}
...
def _required_fit_files(config: dict, stage: str) -> tuple[str, ...]:
    ...
    if config["spatial_method"]:
        common += ("spatial_receipt.json",)
    return common
...
def _fit_input(config: dict, x: torch.Tensor, signal: np.ndarray, meta: pd.DataFrame,
    ...
    if config["spatial_method"]:
        if signal.shape != (5184, 2, 22, 750):
            raise AssertionError("Q10-E001 requires aligned mu/beta EEG in volts")
        train = np.asarray(train, dtype=np.int64)
        actual_subjects = sorted(int(s) for s in meta.iloc[train].subject.unique())
        if actual_subjects != sorted(train_subjects):
            raise AssertionError("Spatial fit subject list differs from source partition")
        required_indices = np.flatnonzero(meta.subject.isin(train_subjects).to_numpy())
        if not np.array_equal(np.sort(train), required_indices):
            raise AssertionError("Spatial fit must use all and only source-training trials")
        projector = fit_projector_pair(
            signal[train, 0], signal[train, 1],
            meta.iloc[train].label.to_numpy(dtype=np.int64),
            meta.iloc[train].subject.to_numpy(dtype=np.int64),
            method=config["spatial_method"],
            source_sample_ids=meta.iloc[train].sample_id.astype(str).to_numpy(),
        )
        receipt = projector.receipt()
        receipt_path = directory / "spatial_receipt.json"
        if receipt_path.exists() and _read_json(receipt_path) != receipt:
            raise AssertionError("Existing source-only spatial receipt differs")
        if not receipt_path.exists():
            write_json(receipt_path, receipt)
        projected = np.empty((len(signal), 16, 750), dtype=np.float32)
        for start in range(0, len(signal), 128):
            stop = min(start + 128, len(signal))
            projected[start:stop] = projector.transform(
                signal[start:stop, 0], signal[start:stop, 1]
            )
        return torch.from_numpy(projected).to(x.device)
...
def _load_data(config: dict, data_dir: Path, device: torch.device, output: Path) -> tuple[torch.Tensor, torch.Tensor, pd.DataFrame, np.ndarray]:
    ...
    values = [bands[name] for name in expected_names]
    if any(arr.shape != (5184, 22, 750) or not np.isfinite(arr).all() for arr in values):
        raise AssertionError("Invalid Q9 EEG shape or values")
    if expected_names != ["mu", "beta"]:
        raise AssertionError("Q10-E001 requires the declared mu/beta band order")
    signal = np.stack(values, axis=1)
    if signal.shape != (5184, 2, 22, 750):
        raise AssertionError("Invalid two-band spatial projection input shape")
    ...
    # The fitted projection is built separately inside each source-only fit.
    # Avoid retaining a second 684 MB copy of raw EEG on the GPU.
...
def _freeze_selection(output: Path, config: dict) -> bool:
    ...
    provenance = {"status": "frozen_before_Q10_target_inference", "rule": "Q8_mean_within_fold_CE_rank",
...
def main() -> None:
    ...
    if args.condition.endswith("_Q8_EPOCHS") and args.phase == "selection":
        parser.error("Fixed-duration controls have no selection phase")
    if args.device == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("CUDA was requested but is unavailable; Q10 GPU run cannot start")
    ...
    if environment_file.exists():
        ...
        for key in ("packages", "torch_cuda_runtime", "cuda_device_name"):
            if old_environment.get(key) != current_environment.get(key):
                raise AssertionError(f"Q10 environment {key} changed during resume")
        if old_environment.get("python", "").split()[0] != current_environment["python"].split()[0]:
            raise AssertionError("Q10 Python version changed during resume")
    else:
        write_json(environment_file, current_environment)
    # Refuse target inference before the 36 source-only inner fits freeze.
    if args.phase == "final" and not config["fixed_q8_epochs"] and not _freeze_selection(output, config):
